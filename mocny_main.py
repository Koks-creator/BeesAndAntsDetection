...
class BeeAntsDetector:
    ...
    @timeit(logger=logger)
    def process_frame(self, frame: List[np.array], use_sahi: bool = True, conf: float = .2, iou: float = .35, 
        ...
        if track:
            ...
            updated_tracks = self.sorttr.update(track_data).astype(int)
            ...
            for track_data in updated_tracks:
                ...
                x1, y1, x2, y2, _, obj_id, class_id = track_data
        ...
        frames_data.append((detection_frame, frame, frame_summary, track_data))
    ...
    def process_video(self, vid_cap: Union[int, Path, str], use_sahi: bool = True, conf: float = .2, 
        ...
        while cap.isOpened():
            ...
            if not success:
                break
            detection_frame, frame, summary_frame, track_data = self.process_frame(
                frame=frame, 
                use_sahi=use_sahi, 
                conf=conf, 
                iou=iou, 
...
if __name__ == "__main__":
    mocne = BeeAntsDetector(
        model_path=Config.MODEL_PATH,
        classes_path=Config.CLASSES_PATH
    )
    mocne.process_video(
        vid_cap=rf"{Config.VIDEOS_FOLDER}/5241046-hd_1280_720_60fps.mp4",
